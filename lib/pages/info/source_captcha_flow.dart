...
class _CaptchaDialogState extends State<_CaptchaDialog> {
  ...
  Future<void> _receiveImage(String? data) async {
    ...
    if (bytes != null) {
      _runOcr(data, revision);
    }
  }

  Future<void> _runOcr(String data, [int? targetRevision]) async {
    if (!CaptchaOcrService.isSupported || _submitting) return;
    final revision = targetRevision ?? _imageRevision;
    setState(() => _recognizing = true);
    final result = await CaptchaOcrService.recognizeCaptcha(data);
    if (!mounted || revision != _imageRevision) return;
          ...
          if (_imageBytes != null) ...[
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: CaptchaOcrService.isSupported && _imageBytes != null
                    ? (_recognizing
                        ? const Padding(
                        ...
                        : IconButton(
                            ...
                            tooltip: '自动识别',
                            onPressed: _submitting || _lastImageData == null
                                ? null
                                : () => _runOcr(_lastImageData!),
                          ))
                    : null,
              ),
            ),
