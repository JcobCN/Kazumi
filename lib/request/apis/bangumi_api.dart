...
class BangumiApi {
  ...
  /// Posts a subject-search request through the Kazumi Bangumi mirror.
  ///
  /// The mirror requires an X-AppId/X-Timestamp/X-Signature trio. The
  /// signature is generated by BangumiClient using the credentials injected
  /// at build time (or the public defaults for self-built clients).
  ///
  /// `includeAuth: false` is intentional. Search is a public endpoint and
  /// must not inherit an expired Bangumi sync token.
  static Future<dynamic> _postSearch(
    ...
    final searchUrl = ApiEndpoints.bangumiMirrorDomain +
        uri.path +
    ...
    try {
      ...
      // Keep the public bgmapi mirror as a fallback for outages or malformed
      // credentials, and bypass Bangumi mirror routing for the retry.
      KazumiLogger().w(
        'BangumiApi: Kazumi mirror search failed, retrying via bgmapi',
        error: e,
      );
      return await _client.post(
